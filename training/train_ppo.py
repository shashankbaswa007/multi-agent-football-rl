...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.epsilon_schedule = LinearSchedule(
            start_value=config.get('epsilon_start', 0.3),
            end_value=config.get('epsilon_end', 0.05),
            duration=config.get('epsilon_decay_episodes', 5000)
        )
    
    def get_current_entropy_coef(self):
        """Get current entropy coefficient with potential override"""
        if self.entropy_schedule:
            return self.entropy_schedule.value(self.episode_count)
        return self.config['ppo_params']['entropy_coef']
    
    def get_current_epsilon(self):
        """Get current epsilon for epsilon-greedy exploration"""
        return self.epsilon_schedule.value(self.episode_count)
    ...
    def collect_episode(self):
        ...
        current_entropy = self.get_current_entropy_coef()
        current_epsilon = self.get_current_epsilon()
        self.team_0_agent.entropy_coef = current_entropy
        self.team_1_agent.entropy_coef = current_entropy
        self.team_0_agent.epsilon_greedy = current_epsilon
        self.team_1_agent.epsilon_greedy = current_epsilon
        ...
        while not done:
            ...
            for agent in self.env.agent_iter():
                observations_dict, rewards_dict, terminations, truncations, info = self.env.step(
                    actions[agent]
                )
                
                episode_rewards[agent] += rewards_dict[agent]
                ...
                buffer.buffers[agent_idx].rewards[buffer.buffers[agent_idx].ptr - 1] = rewards_dict[agent]
    ...
    def update_agents(self, episode):
        ...
        # Note: entropy and epsilon are now updated in collect_episode
        # to ensure consistency during collection
    ...
    def train(self):
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % update_interval == 0:
                ...
                self.writer.add_scalar('Train/entropy_coef', self.get_current_entropy_coef(), episode)
                self.writer.add_scalar('Train/epsilon', self.get_current_epsilon(), episode)
                ...
                if (episode + 1) % (update_interval * 5) == 0:
                    ...
                    print(f"  Entropy Coef: {self.get_current_entropy_coef():.4f}")
